...
export default function TogglePage() {
  ...
  const w = size === 'sm' ? 36 : size === 'md' ? 44 : 52
    ...
    <div style={{ padding: '48px 56px 80px', maxWidth: 900 }}>
      <div style={{ marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <h1 style={{ fontSize: 36, fontWeight: 700, color: '#fff', letterSpacing: '-0.8px' }}>Toggle</h1>
        ...
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.65, maxWidth: 520 }}>
          On/off switch for binary settings. 3 sizes, optional label and disabled state.
        </p>
      </div>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <div style={{ background: '#0F1420', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: 32, flexWrap: 'wrap' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button style={pill(on)} onClick={() => setOn(true)}>on</button>
                <button style={pill(!on)} onClick={() => setOn(false)}>off</button>
              </div>
            </div>
            <div>
              <p style={sL}>Size</p>
          ...
          <div style={{ padding: '60px 40px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 160, background: '#131B2E' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: disabled ? 0.4 : 1, cursor: disabled ? 'not-allowed' : 'pointer' }}
              onClick={() => !disabled && setOn(!on)}>
              <div style={{
                width: w, height: h, borderRadius: 9999,
                background: on ? '#E72757' : 'rgba(255,255,255,0.15)',
                position: 'relative', transition: 'background 0.2s', flexShrink: 0,
              }}>
                <div style={{
                  width: dot, height: dot, borderRadius: '50%', background: '#fff',
                  position: 'absolute', top: 3, left: on ? dotOn : 3,
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '20px 22px' }}>
            ...
            {['Use for settings that take effect immediately without a submit button.','Ideal for turning features on or off — dark mode, notifications, visibility toggles.','Always pair with a label that describes what the toggle controls.'].map(t => (
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '20px 22px' }}>
            ...
            {["Don't use a toggle for actions that require confirmation — use a button instead.","Avoid using toggles for multi-select scenarios; use checkboxes."].map(t => (
              <div key={t} style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', lineHeight: 1.65, marginBottom: 8, paddingLeft: 4 }}>· {t}</div>
            ))}
          </div>
        </div>
